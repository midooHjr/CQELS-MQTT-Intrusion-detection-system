...
public class Overall_MemoryConsumption 
{
    final static Logger logger = Logger.getLogger(Overall_MemoryConsumption.class);
...
/**		reader = new BufferedReader(new FileReader(Settings.QUERY_LIST_FILE_NAME));
    	...
    	StreamPlayer stream=new StreamPlayer(context, "http://www.cwi.nl/SRBench/observations", 
				 ...
				 Config.NUMBER_OF_QUERIES, Config.WINDOW_SIZE, 
...
//        CsvTextStreamer stream = new CsvTextStreamer(context, "http://www.cwi.nl/SRBench/observations", 
//				 Params.STREAM_SOURCE, Params.STREAM_SIZE, 
//				 Params.NUMBER_OF_QUERIES, Params.WINDOW_SIZE, 
//				 Params.STARTING_COUNT, Params.OUTPUT_DES + Params.EXPOUT);
...
//        
		try {
	        ...
	        pw.print(Config.NUMBER_OF_QUERIES + " " + Config.WINDOW_SIZE + " " + avgMem);
	        pw.close();
		} catch (Exception e) {
			e.printStackTrace();
		}
