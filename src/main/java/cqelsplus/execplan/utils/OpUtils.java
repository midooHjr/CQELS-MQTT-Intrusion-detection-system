...
public class OpUtils {
	public static String ptCode(Quad quad) {
		...
		if((quad.getGraph() == null) || (quad.getGraph().isVariable())) {
			...
			result += Long.toString((Long)ExecContextFactory.current().engine().encode(quad.getGraph()));
		}
		
		if(!quad.getSubject().isVariable()) {
			result += Long.toString((Long)ExecContextFactory.current().engine().encode(quad.getSubject()));
		} else {
			result += Long.toString(-1);
		}
		
		if(!quad.getPredicate().isVariable()) {
			result += Long.toString((Long)ExecContextFactory.current().engine().encode(quad.getPredicate()));
		} else {
			result += Long.toString(-1);
		}
		
		if(!quad.getObject().isVariable()) {
			result += Long.toString((Long)ExecContextFactory.current().engine().encode(quad.getObject()));
		} else {
			result += Long.toString(-1);
		}
		return result;
